...
import { Question, MCQQuestion, FillBlankQuestion, MatchingQuestion } from "../../types";
...
export default function QuizPage() {
  ...
  const quiz = getQuizById(quizId) || MOCK_QUIZ;

  const [current, setCurrent] = useState(0);
  const [answers, setAnswers] = useState<Record<number, string | number | Record<string, string>>>({});
  ...
  const checkAnswer = useCallback(() => {
    ...
    switch (q.type) {
      ...
      case "drag-drop":
        ...
        correct = Object.entries(q.correctMapping).every(([k, v]) => mapping[k] === v);
        break;
    }
      ...
      <main className={`min-h-screen ${backgroundClass} ${textClass} relative flex flex-col items-center justify-center px-6 py-12 transition-colors duration-500`}>
        ...
        <div className={`w-full max-w-md rounded-3xl p-8 shadow-xl border-2 text-center ${isDark ? 'bg-slate-900/80 backdrop-blur-xl border-white/10' : 'bg-white border-indigo-100'}`}>
          ...
          <div className={`${isDark ? 'bg-indigo-500/10' : 'bg-indigo-50'} rounded-2xl p-6 mb-6`}>
            ...
            <p className={isDark ? 'text-indigo-300' : 'text-slate-500'}>{score} of {totalQ} correct</p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => router.back()}
              className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl text-lg font-bold shadow-lg shadow-indigo-600/30 bouncy-hover"
            >
              Done
            </button>
          </div>
    ...
    <main className={`min-h-screen ${backgroundClass} ${textClass} relative transition-colors duration-500`}>
      ...
      <div className="px-6 max-w-2xl mx-auto">
        <div className={`${isDark ? 'bg-slate-900/80 backdrop-blur-xl border-white/10 shadow-2xl' : 'bg-white border-indigo-100 shadow-xl'} rounded-3xl p-6 md:p-8 border-2 mb-6`}>
          ...
          <h2 className={`text-xl md:text-2xl font-black mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>{question.question}</h2>
          ...
          {/* Drag & Drop - simplified as select-based for web */}
...
function MCQOptions({ question, selected, onSelect, submitted, isCorrect, isDark }: {
  question: MCQQuestion; selected?: number; onSelect: (idx: number) => void; submitted: boolean; isCorrect: boolean; isDark?: boolean;
}) {
  return (
    <div className="space-y-3">
      {question.options.map((option, idx) => {
        ...
        if (submitted && idx === question.correctIndex) style = isDark ? "bg-emerald-500/30 border-emerald-400 text-emerald-100" : "bg-emerald-100 border-emerald-400 text-emerald-900";
        if (submitted && selected === idx && idx !== question.correctIndex) style = isDark ? "bg-red-500/30 border-red-400 text-red-100" : "bg-red-100 border-red-400 text-red-900";
        ...
        return (
          <button
            ...
            className={`w-full text-left px-5 py-4 rounded-2xl border-2 font-medium transition-all ${style} ${
              !submitted ? "bouncy-hover" : ""
            }`}
          >
            <span className={`font-bold mr-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{String.fromCharCode(65 + idx)}.</span>
            {option}
            {submitted && idx === question.correctIndex && <span className="float-right">✅</span>}
            {submitted && selected === idx && idx !== question.correctIndex && <span className="float-right">❌</span>}
          </button>
        );
      })}
    </div>
...
function FillBlankInput({ question, value, onChange, submitted, isCorrect, isDark }: {
  question: FillBlankQuestion; value: string; onChange: (val: string) => void; submitted: boolean; isCorrect: boolean; isDark?: boolean;
}) {
  return (
    <div>
      <input
        ...
        className={`w-full px-5 py-4 rounded-2xl border-2 text-lg font-medium focus:outline-none ${
          submitted
            ...
            : isDark ? "border-white/10 bg-slate-800/50 text-white focus:border-indigo-400 focus:bg-slate-800" : "border-indigo-100 bg-white text-slate-900 focus:border-indigo-400"
        }`}
        readOnly={submitted}
      />
      {submitted && !isCorrect && (
        <p className={`mt-2 text-sm font-bold ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
...
function MatchingPairs({ question, pairs, onChange, submitted, isCorrect, isDark }: {
  question: MatchingQuestion; pairs: Record<string, string>; onChange: (pairs: Record<string, string>) => void; submitted: boolean; isCorrect: boolean; isDark?: boolean;
}) {
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          {question.leftItems.map((item) => {
            ...
            if (submitted && pairs[item] === question.correctPairs[item]) style = isDark ? "bg-emerald-500/40 border-emerald-400 text-emerald-100" : "bg-emerald-100 border-emerald-400 text-emerald-900";
            if (submitted && pairs[item] && pairs[item] !== question.correctPairs[item]) style = isDark ? "bg-red-500/40 border-red-400 text-red-100" : "bg-red-100 border-red-400 text-red-900";

            return (
              <button
                key={item}
                onClick={() => handleLeftClick(item)}
                className={`w-full px-4 py-3 rounded-xl border-2 font-bold text-sm transition-all ${style}`}
              >
          ...
          {question.rightItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full px-4 py-3 rounded-xl border-2 font-bold text-sm transition-all ${
                  isUsed && !submitted
...
function DragDropSimplified({ question, mapping, onChange, submitted, isCorrect, isDark }: {
  question: { items: string[]; targets: string[]; correctMapping: Record<string, string>; explanation?: string }; mapping: Record<string, string>; onChange: (m: Record<string, string>) => void; submitted: boolean; isCorrect: boolean; isDark?: boolean;
}) {
  return (
    <div>
      ...
      <div className="space-y-3">
        {question.items.map((item) => (
          <div key={item} className="flex items-center gap-3">
            <span className={`border-2 rounded-xl px-4 py-2 font-bold text-sm min-w-[100px] ${isDark ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-200' : 'bg-indigo-50 border-indigo-200 text-indigo-900'}`}>
              {item}
            </span>
            ...
            <select
              ...
              className={`flex-1 px-3 py-2 rounded-xl border-2 font-medium text-sm outline-none ${
                submitted && mapping[item] === question.correctMapping[item]
                  ? isDark ? "border-emerald-500 bg-emerald-900/40 text-emerald-200" : "border-emerald-400 bg-emerald-50 text-emerald-900"
                  : submitted && mapping[item]
                  ? isDark ? "border-red-500 bg-red-900/40 text-red-200" : "border-red-400 bg-red-50 text-red-900"
                  : isDark ? "border-white/10 bg-slate-800/50 text-white focus:border-indigo-400 focus:bg-slate-800" : "border-slate-200 bg-white text-slate-900 focus:border-indigo-400"
              }`}
              disabled={submitted}
            >
